  ...
  queryExamList(dateList.value)
    .then((res) => {
      if (res.code == 200 && res.data.length > 0) {
        ...
        searchData.templateId = res.data[0].templateId;
        queryExamData.value = res.data;
      } else {
        topicType.value = [];
        searchData.examId = "";
...
const examNameHandler = (event) => {
  const templateId = queryExamData.value.find(
    (item) => item.id === event
  ).templateId;
